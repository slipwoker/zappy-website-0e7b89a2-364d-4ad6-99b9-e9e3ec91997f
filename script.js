document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (value !== '') {
        ...
        const minLength = field.minLength && field.minLength > 0 ? field.minLength : null;
        if (minLength && value.length < minLength) {
          showError(field, label + ' must be at least ' + minLength + ' characters.');
          return false;
        }

        if (tag === 'select' && (value === '' || value === 'default')) {
          if (isRequired) {
            showError(field, 'Please select an option for ' + label + '.');
            return false;
          }
        }
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (!isFormValid) {
        ...
        if (firstError) {
          ...
          const navbarEl = document.querySelector('.navbar, .nav, header');
          const navbarHeight = navbarEl ? navbarEl.offsetHeight : 0;
          const top = firstError.getBoundingClientRect().top + window.pageYOffset - navbarHeight - 20;
  ...
  function initScrollAnimations() {
    if (!('IntersectionObserver' in window)) {
      document.querySelectorAll(animatableSelectors.join(',')).forEach(function (el) {
    ...
    elements.forEach(function (el, index) {
      ...
      const delay = el.dataset.delay || (index % 6) * 80;
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "M&H DOGS משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '0e7b89a2-364d-4ad6-99b9-e9e3ec91997f',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '0e7b89a2-364d-4ad6-99b9-e9e3ec91997f';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '0e7b89a2-364d-4ad6-99b9-e9e3ec91997f';
